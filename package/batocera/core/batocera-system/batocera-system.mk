...
BATOCERA_SYSTEM_VERSION = 44-dev-pocket-rc1
...
define BATOCERA_SYSTEM_INSTALL_TARGET_CMDS
	...
	$(INSTALL) -D -m 0755 \
	    $(BR2_EXTERNAL_BATOCERA_PATH)/package/batocera/core/batocera-system/xdg-open \
	    $(TARGET_DIR)/usr/bin/xdg-open
	$(INSTALL) -D -m 0755 \
	    $(BR2_EXTERNAL_BATOCERA_PATH)/package/batocera/core/batocera-system/xdg-mime \
	    $(TARGET_DIR)/usr/bin/xdg-mime
...
define BATOCERA_SYSTEM_INSTALL_XDG_PORTAL_CONFIGS
	mkdir -p $(TARGET_DIR)/usr/share/xdg-desktop-portal
	if [ "$(BR2_PACKAGE_XDG_DESKTOP_PORTAL_GTK)$(BR2_PACKAGE_XDG_DESKTOP_PORTAL_WLR)" = "yy" ]; then \
		if [ "$(BR2_PACKAGE_BATOCERA_WAYLAND_LABWC)" = "y" ]; then \
			$(INSTALL) -D -m 0644 \
				$(BR2_EXTERNAL_BATOCERA_PATH)/package/batocera/core/batocera-system/labwc-portals.conf \
				$(TARGET_DIR)/usr/share/xdg-desktop-portal/labwc-portals.conf; \
		fi; \
		if [ "$(BR2_PACKAGE_BATOCERA_WAYLAND_SWAY)" = "y" ]; then \
			$(INSTALL) -D -m 0644 \
				$(BR2_EXTERNAL_BATOCERA_PATH)/package/batocera/core/batocera-system/sway-portals.conf \
				$(TARGET_DIR)/usr/share/xdg-desktop-portal/sway-portals.conf; \
		fi; \
	fi
endef

BATOCERA_SYSTEM_POST_INSTALL_TARGET_HOOKS += BATOCERA_SYSTEM_INSTALL_XDG_PORTAL_CONFIGS
