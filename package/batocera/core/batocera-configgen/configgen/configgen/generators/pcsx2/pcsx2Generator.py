...
class Pcsx2Generator(Generator):
    ...
    # PCSX2 crashes if MangoHUD is injected
...
def configureINI(config_directory: Path, bios_directory: Path, system: Emulator, rom: Path, controllers: Controllers, metadata: Mapping[str, str], guns: Guns, wheels: DeviceInfoMapping, playingWithWheel: bool) -> None:
    ...
    # EE cycle rate / underclock-overclock controls
    pcsx2INIConfig.set("EmuCore/Speedhacks", "EECycleRate", system.config.get("pcsx2_ee_cycle_rate", "0"))
    ...
    # HW Download mode (GPU to CPU sync behavior)
    pcsx2INIConfig.set("EmuCore/GS", "HWDownloadMode", system.config.get("pcsx2_hw_download", "0"))
