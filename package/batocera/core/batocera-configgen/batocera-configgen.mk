...
define BATOCERA_CONFIGGEN_CONFIGS
	...
	install -D -m 0755 $(CONFIGGEN_DIR)/scripts/call_achievements_hooks.sh \
	    $(TARGET_DIR)/usr/share/batocera/configgen/call_achievements_hooks.sh
...
define BATOCERA_CONFIGGEN_PORTMASTER_PY311_COMPAT
	# PortMaster variants may hardcode /usr/lib/python3.11/site-packages/configgen.
	# Provide compatibility symlinks for the module path and interpreter name.
	PYCFG_DIR="$$(ls -d $(TARGET_DIR)/usr/lib/python3*/site-packages/configgen 2>/dev/null | sort -V | tail -n1)"; \
	if [ -n "$$PYCFG_DIR" ]; then \
		PYCFG_TARGET="$${PYCFG_DIR#$(TARGET_DIR)}"; \
		mkdir -p $(TARGET_DIR)/usr/lib/python3.11/site-packages; \
		ln -snf "$$PYCFG_TARGET" $(TARGET_DIR)/usr/lib/python3.11/site-packages/configgen; \
	fi
	if [ -x "$(TARGET_DIR)/usr/bin/python3" ] && [ ! -e "$(TARGET_DIR)/usr/bin/python3.11" ]; then \
		ln -snf python3 $(TARGET_DIR)/usr/bin/python3.11; \
	fi
endef
...
BATOCERA_CONFIGGEN_POST_INSTALL_TARGET_HOOKS += BATOCERA_CONFIGGEN_PORTMASTER_PY311_COMPAT
