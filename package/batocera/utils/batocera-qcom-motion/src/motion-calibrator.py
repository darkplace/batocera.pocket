...
POSES = (
    ...
    ("left", "Stand on LEFT edge", (1.0, 0.0, 0.0)),
    ("right", "Stand on RIGHT edge", (-1.0, 0.0, 0.0)),
    ("top", "Stand on TOP edge", (0.0, 0.0, 1.0)),
...
def calculate_accelerometer_calibration(
    ...
    # Constrain calibration to independent axis offsets and gains. A general
    # affine fit can turn a few degrees of hand-held pose error into shear,
    # which makes gyro integration disagree with the gravity vector. Opposing
    # edge pairs give reliable X/Z zero points; Y uses the face-up magnitude
    # and deliberately assumes zero offset so no face-down pose is needed.
    offset = (
        (left[0] + right[0]) * 0.5,
        0.0,
        (top[2] + bottom[2]) * 0.5,
    ...
    correction = [[0.0, 0.0, 0.0] for _ in range(3)]
    for axis, span in enumerate(spans):
        correction[axis][axis] = 1.0 / span
...
class MotionCalibrator:
    ...
    def cancel(self) -> None:
        if self.collecting:
            self.collecting = False
            self.pose_samples.clear()
            self.status = "Pose canceled; press A when ready"
            return
        self.quit()
    ...
    def update(self) -> None:
        ...
        if self.state == "pose":
            ...
            if not self.collecting and self.pose_index == 0:
                self.begin_pose()
            elif not self.collecting and self.confirm_pressed():
                self.begin_pose()
    ...
    def draw_device(self, pose: str) -> None:
        ...
        if pose in {"face_up", "face_down"}:
            ...
            if pose == "face_up":
                ...
                pyxel.text(137, 111, "SCREEN UP", 7)
            else:
    ...
    def draw(self) -> None:
        ...
        if self.state in {"intro", "pose", "validation"}:
