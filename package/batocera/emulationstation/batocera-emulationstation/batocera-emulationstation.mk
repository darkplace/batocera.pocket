...
#
################################################################################
# Last update: Commits on May 21, 2026
BATOCERA_EMULATIONSTATION_VERSION = 71a7dbe1f03ed0746c0fc5e9d7e8955af08fb8e0
BATOCERA_EMULATIONSTATION_SITE = https://github.com/darkplace/batocera-emulationstation
...
BATOCERA_EMULATIONSTATION_CXX_FLAGS = -D$(call UPPERCASE,$(BATOCERA_SYSTEM_ARCH))

ifeq ($(BR2_PACKAGE_WAYLAND),y)
BATOCERA_EMULATIONSTATION_CXX_FLAGS += -DBATOCERA_NIGHTMODE -DBATOCERA_OLEDCARE
endif

BATOCERA_EMULATIONSTATION_CONF_OPTS += \
    "-DCMAKE_CXX_FLAGS=$(BATOCERA_EMULATIONSTATION_CXX_FLAGS)"
...
BATOCERA_EMULATIONSTATION_DEFAULT_CREDENTIALS_FILE = \
    $(firstword \
	$(wildcard $(BATOCERA_EMULATIONSTATION_SOURCE_PATH)/api-credentials.conf) \
	$(wildcard $(BATOCERA_EMULATIONSTATION_SOURCE_PATH)/keys.txt) \
	/dev/null)
BATOCERA_EMULATIONSTATION_CREDENTIALS_FILE ?= \
    $(BATOCERA_EMULATIONSTATION_DEFAULT_CREDENTIALS_FILE)

BATOCERA_EMULATIONSTATION_KEY_SCREENSCRAPER_DEV_LOGIN=\
    $(shell grep -E '^SCREENSCRAPER_DEV_LOGIN=' \
	$(BATOCERA_EMULATIONSTATION_CREDENTIALS_FILE) 2>/dev/null | cut -d = -f 2-)
...
BATOCERA_EMULATIONSTATION_PREFIX = SDL_NOMOUSE=1 SDL_VIDEODRIVER=wayland SDL_RENDER_VSYNC=1
BATOCERA_EMULATIONSTATION_CMD = sway-launch
BATOCERA_EMULATIONSTATION_ARGS += --vsync on
...
define BATOCERA_EMULATIONSTATION_WAYLAND_SWAY
	rm -f $(TARGET_DIR)/etc/profile.d/04-labwc.sh
	rm -f $(TARGET_DIR)/etc/init.d/S14labwc
	rm -f $(TARGET_DIR)/usr/bin/labwc-launch
	rm -rf $(TARGET_DIR)/usr/share/labwc
    ...
    $(INSTALL) -D -m 0755 $(BATOCERA_EMULATIONSTATION_SOURCE_PATH)/wayland/sway/sway-touch-map \
	    $(TARGET_DIR)/usr/bin/sway-touch-map
endef

define BATOCERA_EMULATIONSTATION_WAYLAND_LABWC
	rm -f $(TARGET_DIR)/etc/profile.d/04-sway.sh
	rm -f $(TARGET_DIR)/etc/sway/config
	rm -f $(TARGET_DIR)/usr/bin/sway-launch
	rm -f $(TARGET_DIR)/usr/bin/sway-touch-map
