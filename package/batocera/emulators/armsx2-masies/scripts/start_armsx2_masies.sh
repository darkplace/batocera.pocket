...
if [ ! -f "${DATA_ROOT}/game_controller_db.txt" ]; then
	...
	elif [ -f /usr/armsx2-masies/bin/resources/game_controller_db.txt ]; then
		cp -f /usr/armsx2-masies/bin/resources/game_controller_db.txt "${DATA_ROOT}/game_controller_db.txt"
	else
...
cd /usr/armsx2-masies/bin

exec /usr/armsx2-masies/bin/pcsx2-qt \
	-nogui \
	-fullscreen \
	-logfile /userdata/system/logs/armsx2-masies.log \
	-- "${ROM}"
