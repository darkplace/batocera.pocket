#!/usr/bin/env bash
# Rebuild gamescope (wl_touch) + onscreen-keyboard layers, remake sm8550 OTA images.
set -euo pipefail
PROJECT_DIR="$(cd "$(dirname "$0")/../.." && pwd)"
cd "$PROJECT_DIR"

LOG_FILE="${LOG_FILE:-$PROJECT_DIR/rebuild-sm8550-gamescope-osk-ota.log}"
...
log() { printf '%s\n' "$*" | tee -a "$LOG_FILE"; }
: >"$LOG_FILE"
rm -f "$PROJECT_DIR/rebuild-sm8550-gamescope-osk-ota.DONE" \
      "$PROJECT_DIR/rebuild-sm8550-gamescope-osk-ota.FAILED"
...
run_pkg() {
  ...
  if [ "$ec" -ne 0 ]; then
    log "FAILED: $pkg ($ec)"
    echo "FAILED $pkg $(date -Is)" >"$PROJECT_DIR/rebuild-sm8550-gamescope-osk-ota.FAILED"
    exit "$ec"
  fi
  log "OK: $pkg"
}
...
run_cmd() {
  ...
  if [ "$ec" -ne 0 ]; then
    ...
    echo "FAILED CMD=$cmd $(date -Is)" >"$PROJECT_DIR/rebuild-sm8550-gamescope-osk-ota.FAILED"
    exit "$ec"
  fi
  log "OK: CMD=$cmd"
}

log "=== sm8550 gamescope wl_touch + OSK OTA ==="
...
if [ ! -f "$PATCH" ]; then
  ...
  echo "FAILED missing-patch $(date -Is)" >"$PROJECT_DIR/rebuild-sm8550-gamescope-osk-ota.FAILED"
  exit 1
fi
if [ -f "${PATCH}.disabled" ]; then
  log "FAIL: patch still disabled (${PATCH}.disabled)"
  echo "FAILED patch-disabled $(date -Is)" >"$PROJECT_DIR/rebuild-sm8550-gamescope-osk-ota.FAILED"
  exit 1
fi
if ! grep -q 'simple,special' \
  ...
  echo "FAILED osk-layers $(date -Is)" >"$PROJECT_DIR/rebuild-sm8550-gamescope-osk-ota.FAILED"
  exit 1
fi
...
if [ ! -x "$GS" ]; then
  log "FAIL: missing $GS"
  echo "FAILED missing-gamescope $(date -Is)" >"$PROJECT_DIR/rebuild-sm8550-gamescope-osk-ota.FAILED"
  exit 1
fi
if ! nm -C "$GS" 2>/dev/null | grep -q 'Wayland_Touch_Down' \
  ...
  if ! grep -Rq 'Wayland_Touch_Down' "$PRIMARY"/build/gamescope-*/src/Backends/WaylandBackend.cpp 2>/dev/null; then
    ...
    echo "FAILED no-wl-touch $(date -Is)" >"$PROJECT_DIR/rebuild-sm8550-gamescope-osk-ota.FAILED"
    exit 1
  fi
fi
...
if ! grep -q 'simple,special' "$OSK"; then
  ...
  echo "FAILED osk-target $(date -Is)" >"$PROJECT_DIR/rebuild-sm8550-gamescope-osk-ota.FAILED"
  exit 1
fi
...
echo "OK $(date -Is)" >"$PROJECT_DIR/rebuild-sm8550-gamescope-osk-ota.DONE"
log "DONE — next: split-release + gh release v44-sm8550-20260816"
