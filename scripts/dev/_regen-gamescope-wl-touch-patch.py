#!/usr/bin/env python3
"""Regenerate wl_touch patch against clean gamescope 3.16.25 tree."""
...
CLEAN = Path("/tmp/gamescope-315/src/Backends/WaylandBackend.cpp")
OUT_PATCH = Path(
    "/home/lukemotion/batocera.pocket/package/batocera/utils/gamescope"
    "/001-rocknix-wayland-touch.patch"
)
WORK = Path("/tmp/gs31625-patch/src/Backends/WaylandBackend.cpp")
...
def main() -> None:
    if not CLEAN.is_file():
        raise SystemExit(f"missing clean source: {CLEAN}")
    ...
    impl = (
        "    // Touch\n\n"
        "    void CWaylandInputThread::Wayland_Touch_Down( wl_touch *pTouch, uint32_t uSerial, "
        "uint32_t uTime, wl_surface *pSurface, int32_t nId, wl_fixed_t fX, wl_fixed_t fY )\n"
        "    {\n"
        ...
        "        wlserver_unlock();\n"
        "    }\n"
        "    void CWaylandInputThread::Wayland_Touch_Up( wl_touch *pTouch, uint32_t uSerial, "
        "uint32_t uTime, int32_t nId )\n"
        "    {\n"
        "        wlserver_lock();\n"
        "        wlserver_touchup( nId, ++m_uFakeTimestamp );\n"
        "        wlserver_unlock();\n"
        "    }\n"
        "    void CWaylandInputThread::Wayland_Touch_Motion( wl_touch *pTouch, uint32_t uTime, "
        "int32_t nId, wl_fixed_t fX, wl_fixed_t fY )\n"
        "    {\n"
        ...
        "    }\n"
        "    void CWaylandInputThread::Wayland_Touch_Shape( wl_touch *pTouch, int32_t nId, "
        "wl_fixed_t fMajor, wl_fixed_t fMinor )\n"
        "    {\n"
        "    }\n"
        "    void CWaylandInputThread::Wayland_Touch_Orientation( wl_touch *pTouch, int32_t nId, "
        "wl_fixed_t fOrientation )\n"
        "    {\n"
        "    }\n\n"
        "    /////////////////////////\n"
    ...
    print(f"regenerated {OUT_PATCH} ({OUT_PATCH.stat().st_size} bytes)")
