#!/usr/bin/env bash
# Rebuild sm8550 "Golden Rabbit" OTA image:
#   same Steam Proton11/MangoApp + wine-tools cut as sm8750 Golden Rabbit,
#   on top of the existing sm8550 LED/rsinput baseline.
set -euo pipefail
PROJECT_DIR="$(cd "$(dirname "$0")/../.." && pwd)"
cd "$PROJECT_DIR"

LOG_FILE="${LOG_FILE:-$PROJECT_DIR/rebuild-sm8550-golden-rabbit.log}"
...
log() { printf '%s\n' "$*" | tee -a "$LOG_FILE"; }
: >"$LOG_FILE"
rm -f "$PROJECT_DIR/rebuild-sm8550-golden-rabbit.DONE" \
      "$PROJECT_DIR/rebuild-sm8550-golden-rabbit.FAILED"
...
run_pkg() {
  ...
  if [ "$ec" -ne 0 ]; then
    log "FAILED: $pkg ($ec)"
    echo "FAILED $pkg $(date -Is)" >"$PROJECT_DIR/rebuild-sm8550-golden-rabbit.FAILED"
    exit "$ec"
  fi
  log "OK: $pkg"
}
...
run_cmd() {
  ...
  if [ "$ec" -ne 0 ]; then
    ...
    echo "FAILED CMD=$cmd $(date -Is)" >"$PROJECT_DIR/rebuild-sm8550-golden-rabbit.FAILED"
    exit "$ec"
  fi
  log "OK: CMD=$cmd"
}

log "=== sm8550 Golden Rabbit rebuild ==="
log "Date: $(date -Is) PID=$$"
log "Codename: Golden Rabbit (44-dev-pocket-golden-rabbit)"
log "Tag target: v44-sm8550-20260818"
...
echo "OK $(date -Is)" >"$PROJECT_DIR/rebuild-sm8550-golden-rabbit.DONE"
